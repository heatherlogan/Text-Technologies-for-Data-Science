...
# epresenting documents to work on when  building indewx
    ...
    for document in split_file():
        ...
        for word in tokenizedline:
            if word not in stopwords:
                processed_text.append(stem(word.lower()))
...
def build_index(preprocessed_file, terms):
    print("beginning index building")
    inverted_index = []
    #loop through each term
    for term in terms:
        all_positions = []
        for document in preproccessed_file[:20]:
